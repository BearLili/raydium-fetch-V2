...
async function init() {
  ...
  const pool_keys_array = await fetchPoolKeys(publicKeyArr)

  return { publicKeyArr, pool_keys_array }
}

// use 「pubilcKeyList」 and 「tokenList」 to get 「pool-KeysList」,include everyToken's Revece;
async function fetchPoolKeys(poolId, version = 4) {
...
async function fetchPoolInfo(pool_keys_array, config_tokenArr) {
  try {
    ...
    const results = await compute(pool_keys_array, new_pool_keys_array)
    ...
    // new Fetch
    await fetchPoolInfo(pool_keys_array, config_tokenArr)
  } catch (err) {
    main()
  }
}
...
async function compute(poolKeysList, new_pool_keys_array) {
  try {
...
async function getClmmPoolInfo() {
  const id = new PublicKey('2QdhepnKRTLjjSqPL1PtKNwqrUkoLee5Gqs8bvZhRdMv')
